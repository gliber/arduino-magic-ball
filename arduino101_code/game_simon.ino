...
void setLEDs(byte leds)
{
  ...
  {
    effectQuarter(1, BLACK);
    effectQuarter(2, BLACK);
  }
  if ((leds & CHOICE_GREEN) != 0)
  {
    effectQuarter(3, GREEN);
    effectQuarter(4, GREEN);
  }
  else
  {
    effectQuarter(3, BLACK);
    effectQuarter(4, BLACK);
  }

  if ((leds & CHOICE_YELLOW) != 0)
  {
    effectQuarter(5, YELLOW);
    effectQuarter(6, YELLOW);
  }
  else
  {
    effectQuarter(5, BLACK);
    effectQuarter(6, BLACK);
  }

  if ((leds & CHOICE_BLUE) != 0)
  {
    effectQuarter(7, BLUE);
    effectQuarter(0, BLUE);
  }
  else
  {
    effectQuarter(7, BLACK);
    effectQuarter(0, BLACK);
  }
}
...
//based on the CurieIMU\AccelerometerOrientation sample

  // calculate the absolute values, to determine the largest
  float accelX, accelY, accelZ;   //scaled accelerometer values
  getAcceleromter(accelX,accelY,accelZ);
...
void toner(byte which, int buzz_length_ms)
{
  ...
  case CHOICE_RED:
    ...
    buzz_sound(buzz_length_ms, 440); 
    break;
  case CHOICE_GREEN:
    //buzz_sound(buzz_length_ms, 568); 
    buzz_sound(buzz_length_ms, 880); 
    break;
  case CHOICE_BLUE:
    //buzz_sound(buzz_length_ms, 851); 
    buzz_sound(buzz_length_ms, 587); 
    break;
  case CHOICE_YELLOW:
    //buzz_sound(buzz_length_ms, 638); 
    buzz_sound(buzz_length_ms, 784); 
    break;
  }
...
void buzz_sound(int duration, int freq)
{
  /*
  ...
  */
  tone(ICBALL_BUZZER, freq,duration );
  delay(duration);
}
